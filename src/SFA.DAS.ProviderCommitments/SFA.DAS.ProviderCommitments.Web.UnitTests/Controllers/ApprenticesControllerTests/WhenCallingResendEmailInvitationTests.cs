...
public class WhenCallingResendEmailInvitationTestsFixture : ApprenticeControllerTestFixtureBase
{
    ...
    public void VerifyRedirect(IActionResult result)
    {
        ...
        {
            _request.ProviderId.Should().Be(long.Parse(redirect.RouteValues["ProviderId"].ToString()));
            _request.ApprenticeshipHashedId.Should().Be(redirect.RouteValues["ApprenticeshipHashedId"].ToString());
        }
    }
}