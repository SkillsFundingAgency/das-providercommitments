...
public class SelectDeliveryModelViewModelMapperTests
{
    ...
    public async Task LegalEntityName_Is_Mapped_Correctly()
    {
        ...
        result.LegalEntityName.Should().Be(_apiResponse.LegalEntityName);
    }
    ...
    {
        var result = await _mapper.Map(_request);
        result.ApprenticeshipStatus.Should().Be(_apiResponse.Status);
    }
    ...
    {
        var result = await _mapper.Map(_request);
        result.DeliveryModel.Should().Be(_cacheItem.DeliveryModel);
    }
    ...
    public async Task When_Only_One_DeliveryModel_Option_Is_Available_It_Is_Automatically_Persisted_To_Cache()
    {
        ...
        _cacheItem.DeliveryModel.Should().Be(deliveryModel);
    }
}